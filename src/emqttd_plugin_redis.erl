...
load() ->
	lager:error("Init authcmd=~s, client_file=~s", [env(authcmd), env(clients_file)]),
    SuperCmd = application:get_env(?APP, supercmd, undefined),
    ok = emqttd_access_control:register_mod(
            auth, emqttd_auth_redis, {SuperCmd, env(authcmd), env(password_hash), env(clients_file)}),
...
on_client_connected(?CONNACK_ACCEPT, Client = #mqtt_client{client_pid = ClientPid}, SubCmd) ->
	lager:debug("Query subscriptions: ~p, Cmd: ~p", [Client#mqtt_client.client_id, SubCmd]),
